...
import { ArrowLeft, ArrowRight, Check, ChevronDown, ChevronUp, CircleDollarSign, FileCheck2, Gauge, LockKeyhole, MapPin, Menu, RotateCcw, Send, ShieldCheck, Sparkles, X } from "lucide-react";
  ...
  return <div dir={dir} className="min-h-screen bg-[#07111f] text-[#f8f3e7] selection:bg-[#d7b45a] selection:text-[#07111f]">
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#07111f]/95 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-3 lg:px-8">
        ...
        <div className="flex items-center gap-2"><Button variant="ghost" onClick={() => setLang(lang === "ar" ? "en" : "ar")} className="text-xs text-white/70 hover:bg-white/10 hover:text-white">{lang === "ar" ? "EN" : "عربي"}</Button><Button onClick={() => document.getElementById("valuation")?.scrollIntoView({ behavior: "smooth" })} className="hidden bg-[#d7b45a] text-[#07111f] hover:bg-[#edd078] sm:inline-flex">{t.start}</Button><Button variant="ghost" aria-label={mobileMenuOpen ? (lang === "ar" ? "إغلاق القائمة" : "Close menu") : (lang === "ar" ? "فتح القائمة" : "Open menu")} aria-expanded={mobileMenuOpen} onClick={() => setMobileMenuOpen((open) => !open)} className="inline-flex h-10 w-10 p-0 text-[#d7b45a] hover:bg-white/10 hover:text-[#efd47d] md:hidden">{mobileMenuOpen ? <X size={22}/> : <Menu size={22}/>}</Button></div>
      </div>
      <div className={`${mobileMenuOpen ? "block" : "hidden"} border-t border-white/10 bg-[#0b192b] md:hidden`}>
        <nav className="mx-auto grid max-w-7xl gap-1 px-5 py-3 lg:px-8">{t.nav.map((item, index) => <a key={item} href={index === 1 ? "#valuation" : index === 2 ? "#how" : "#top"} onClick={() => setMobileMenuOpen(false)} className="rounded-lg px-4 py-3 text-sm text-white/75 transition-colors hover:bg-white/10 hover:text-[#efd47d]">{item}</a>)}<Button onClick={() => { setMobileMenuOpen(false); document.getElementById("valuation")?.scrollIntoView({ behavior: "smooth" }); }} className="mt-2 w-full bg-[#d7b45a] text-[#07111f] hover:bg-[#edd078] sm:hidden">{t.start}</Button></nav>
      </div>
    </header>

...
import { ArrowLeft, ArrowRight, Check, ChevronDown, ChevronUp, CircleDollarSign, FileCheck2, Gauge, ImagePlus, LockKeyhole, MapPin, Menu, RotateCcw, Send, ShieldCheck, Sparkles, UploadCloud, X } from "lucide-react";
...
import { getImageIdentity, MAX_IMAGE_BYTES, MAX_PROPERTY_IMAGES } from "@shared/imageValidation";
import { moveImage, processImageBatch, selectImageFiles } from "@shared/imageUpload";
import { mobileNavigationReducer } from "@/lib/mobileNavigation";

type Lang = "ar" | "en";
type ImagePreview = { id: string; name: string; url: string; size: number; blob: Blob; identity: string };
type FormState = {
...
export default function Home() {
  ...
  const imageUpload = trpc.valuationImages.useMutation({ onError: () => toast.error(lang === "ar" ? "تعذر حفظ صور العقار، بينما تم حفظ التقييم." : "Images could not be saved, but the valuation was saved.") });
  const valuation = trpc.valuation.submit.useMutation({
    onSuccess: async (data) => { setResult(data); setServerErrors({}); setStep(6); toast.success(t.saved); if (form.consent && images.length > 0) { const imagePayload = await Promise.all(images.map(async (image, position) => ({ originalName: image.name, mimeType: image.blob.type as "image/jpeg" | "image/png" | "image/webp", sizeBytes: image.blob.size, position, dataBase64: await blobToBase64(image.blob) }))); imageUpload.mutate({ valuationRef: data.valuationRef, images: imagePayload }); } },
    onError: (error) => {
  ...
  const faq = lang === "ar" ? [{ q: "هل النتيجة تقييم رسمي؟", a: "لا. النتيجة تقدير إرشادي مبدئي وليست تقرير تقييم معتمداً أو بديلاً عن المثمن المرخص." }, { q: "كيف تُحسب القيمة؟", a: "يستخدم المحرك الموقع ونوع العقار والمساحة والعمر والحالة ومدخلات السوق والمقارنات الموثقة المتاحة." }, { q: "هل تُحفظ بياناتي؟", a: "تُتحقق البيانات خادمياً وتُحفظ مع مرجع فريد وفق إعدادات المنصة، ولا ينبغي إدخال بيانات حساسة غير مطلوبة." }] : [{ q: "Is this a licensed appraisal?", a: "No. It is an indicative estimate and does not replace a licensed appraiser or certified valuation report." }, { q: "How is the estimate calculated?", a: "The engine uses location, property type, area, age, condition, market inputs, and available verified comparables." }, { q: "Are my details stored?", a: "Inputs are validated server-side and stored with a unique reference according to the platform settings. Do not enter unnecessary sensitive data." }];
  ...
  function reset() { setForm(initialForm); setImages([]); setResult(undefined); setStep(1); valuation.reset(); }
  function navHref(index: number) { return index === 1 ? "#valuation" : index === 2 ? "#how" : "#top"; }
  function goTo(id: string) { dispatchMobileMenu({ type: "close" }); document.getElementById(id)?.scrollIntoView({ behavior: "smooth" }); }
  useEffect(() => {
    if (!mobileMenuOpen) return;
    const onKeyDown = (event: KeyboardEvent) => { if (event.key === "Escape") dispatchMobileMenu({ type: "close" }); };
  ...
  return <div dir={dir} className="min-h-screen bg-[#07111f] text-[#f8f3e7] selection:bg-[#d7b45a] selection:text-[#07111f]">
    <header className="sticky top-0 z-30 border-b border-white/10 bg-[#07111f]/95 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-5 sm:py-4 lg:px-8">
        <a href="#top" onClick={() => dispatchMobileMenu({ type: "close" })} className="flex min-w-0 items-center gap-3" aria-label={lang === "ar" ? "العودة إلى الرئيسية" : "Back to home"}><div className="flex h-10 w-10 shrink-0 items-center justify-center border border-[#d7b45a]/60 text-[#d7b45a]"><Sparkles size={18}/></div><div><div className="text-sm font-semibold tracking-[0.22em] text-[#d7b45a]">HIMMAT</div><div className="text-[10px] tracking-[0.3em] text-white/45">AL MADINAH</div></div></a>
        <nav aria-label={lang === "ar" ? "التنقل الرئيسي" : "Main navigation"} className="hidden items-center gap-8 text-sm text-white/60 md:flex">{t.nav.map((item, index) => <a key={item} href={navHref(index)} className="transition-colors hover:text-[#d7b45a]">{item}</a>)}</nav>
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <Button onClick={() => goTo("valuation")} className="hidden bg-[#d7b45a] text-[#07111f] hover:bg-[#edd078] sm:inline-flex">{t.start}</Button>
          <Button variant="ghost" size="icon" onClick={() => dispatchMobileMenu({ type: "toggle" })} className="inline-flex h-10 w-10 border border-white/15 text-white hover:bg-white/10 hover:text-[#d7b45a] md:hidden" aria-expanded={mobileMenuOpen} aria-controls="mobile-navigation" aria-label={mobileMenuOpen ? (lang === "ar" ? "إغلاق القائمة" : "Close menu") : (lang === "ar" ? "فتح القائمة" : "Open menu")}>
            {mobileMenuOpen ? <X size={21}/> : <Menu size={21}/>}
          </Button>
        </div>
      </div>
      <div id="mobile-navigation" className={`${mobileMenuOpen ? "block" : "hidden"} border-t border-white/10 bg-[#0b192b] md:hidden`}>
        <nav aria-label={lang === "ar" ? "قائمة الجوال" : "Mobile navigation"} className="mx-auto flex max-w-7xl flex-col px-4 py-3 sm:px-5">
          {t.nav.map((item, index) => <a key={item} href={navHref(index)} onClick={() => dispatchMobileMenu({ type: "close" })} className="flex min-h-12 items-center border-b border-white/10 px-2 text-sm text-white/80 last:border-b-0 hover:bg-white/[.04] hover:text-[#d7b45a]">{item}</a>)}
          <Button onClick={() => goTo("valuation")} className="mt-3 min-h-11 w-full bg-[#d7b45a] text-[#07111f] hover:bg-[#edd078]">{t.start}</Button>
        </nav>
      </div>
    </header>
    ...
    <main id="top">
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 lg:px-8"><div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr]"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#d7b45a]">{lang === "ar" ? "أسئلة شائعة" : "FAQ"}</p><h2 className="mt-4 text-3xl font-semibold">{lang === "ar" ? "وضوح قبل القرار" : "Clarity before a decision"}</h2><p className="mt-4 leading-7 text-white/55">{lang === "ar" ? "إجابات مختصرة على أكثر الأسئلة شيوعاً حول النتيجة والبيانات." : "Short answers to common questions about estimates and data."}</p></div><div className="divide-y divide-white/10 border-y border-white/10">{faq.map((item, index) => <div key={item.q}><button type="button" aria-expanded={openFaq === index} onClick={() => setOpenFaq(openFaq === index ? null : index)} className="flex w-full items-center justify-between gap-4 py-5 text-start text-sm font-semibold text-white"><span>{item.q}</span>{openFaq === index ? <ChevronUp size={18} className="shrink-0 text-[#d7b45a]"/> : <ChevronDown size={18} className="shrink-0 text-[#d7b45a]"/>}</button>{openFaq === index && <p className="pb-5 pe-8 text-sm leading-7 text-white/55">{item.a}</p>}</div>)}</div></div></section>
      <section className="mx-auto max-w-7xl px-5 pb-16 lg:px-8"><div className="border border-white/10 bg-white/[.025] p-6 md:p-8"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#d7b45a]">{lang === "ar" ? "إضافة عقارات متعددة" : "Bulk property import"}</p><h2 className="mt-3 text-xl font-semibold">{lang === "ar" ? "قم برفع ملف Excel لبيانات عقارات متعددة دفعة واحدة" : "Upload an Excel file for multiple properties at once"}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-white/55">{lang === "ar" ? "استخدم الأعمدة: العنوان، المدينة، الحي، نوع العقار، المساحة، العمر، الحالة، السعر، تاريخ البيع، والمصدر. نزّل النموذج لمعرفة التنسيق المطلوب." : "Use columns for title, city, district, property type, area, age, condition, price, sale date, and source. Download the template to follow the required format."}</p></div><a href="/property-import-template.csv" download className="inline-flex shrink-0 items-center justify-center border border-[#d7b45a]/45 px-5 py-3 text-sm font-semibold text-[#efd47d] hover:bg-[#d7b45a]/10">{lang === "ar" ? "تنزيل نموذج البيانات" : "Download data template"}</a></div></div></section>
    </main>
...
function ResultPanel({ result, t, lang }: { result: NonNullable<Awaited<ReturnType<typeof trpc.valuation.submit.useMutation>>["data"]>; t: typeof copy.ar; lang: Lang }) { return <div><div className="border border-[#d7b45a]/40 bg-[#d7b45a]/[.08] p-6 md:p-8"><div className="text-xs font-semibold uppercase tracking-[0.22em] text-[#d7b45a]">{lang === "ar" ? "القيمة الإرشادية" : "Indicative point estimate"}</div><div className="mt-3 text-4xl font-semibold tracking-tight text-[#efd47d] md:text-6xl">{money(result.pointPrice, lang)}</div><p className="mt-3 text-sm leading-6 text-white/55">{t.disclaimer}</p></div><div className="mt-5 grid gap-px bg-white/10 sm:grid-cols-3"><Metric label={t.range} value={`${money(result.lowPrice, lang)} — ${money(result.highPrice, lang)}`}/><Metric label={t.confidence} value={percent(result.confidence)}/><Metric label={t.ref} value={result.valuationRef}/></div><div className="mt-8"><h3 className="mb-4 text-xl font-semibold">{t.factors}</h3><div className="grid gap-3 sm:grid-cols-2">{result.factors.map((factor) => <div key={factor.key} className="border border-white/10 bg-white/[.02] p-4"><div className="flex justify-between gap-3"><span className="text-white/55">{factor.label}</span><span className="text-[#efd47d]">{factor.impact}</span></div><div className="mt-2 text-sm text-white/80">{factor.detail}</div></div>)}</div></div><div className="mt-8 border-t border-white/10 pt-5 text-sm leading-7 text-white/55">{result.disclaimer}</div></div>; }

function ImageUploader({ lang, images, setImages, title, hint, note, removeLabel }: { lang: Lang; images: ImagePreview[]; setImages: React.Dispatch<React.SetStateAction<ImagePreview[]>>; title: string; hint: string; note: string; removeLabel: string }) {
  const [uploadErrors, setUploadErrors] = useState<string[]>([]);
  async function addFiles(fileList: FileList | null) {
    if (!fileList) return;
    const selected = Array.from(fileList);
    const errors: string[] = [];
    const selection = selectImageFiles(selected, images.map((image) => ({ name: image.name, size: image.identity.split("|")[1] ? Number(image.identity.split("|")[1]) : image.size, lastModified: Number(image.identity.split("|")[2]), type: "image/jpeg" })), MAX_PROPERTY_IMAGES);
    for (const issue of selection.issues) {
      if (issue.code === "limit") errors.push(lang === "ar" ? `يمكن إضافة ${MAX_PROPERTY_IMAGES} صور كحد أقصى.` : `You can add up to ${MAX_PROPERTY_IMAGES} images.`);
      if (issue.code === "unsupported") errors.push(lang === "ar" ? `${issue.name}: نوع الملف غير مدعوم أو حجمه يتجاوز ${Math.round(MAX_IMAGE_BYTES / 1024 / 1024)}MB.` : `${issue.name}: unsupported type or larger than ${Math.round(MAX_IMAGE_BYTES / 1024 / 1024)}MB.`);
      if (issue.code === "duplicate") errors.push(lang === "ar" ? `${issue.name}: الصورة مضافة مسبقاً أو مكررة.` : `${issue.name}: duplicate image.`);
    }
    const valid = selection.accepted;
    const compressed = await processImageBatch(valid, async (file) => { const blob = await compressImage(file); return { id: `${file.name}-${file.lastModified}-${Math.random()}`, name: file.name, url: URL.createObjectURL(blob), size: blob.size, blob, identity: getImageIdentity(file) }; }, (file) => errors.push(lang === "ar" ? `${file.name}: تعذر قراءة أو ضغط الصورة.` : `${file.name}: image could not be read or compressed.`));
    setUploadErrors(errors);
    setImages((current) => [...current, ...compressed].slice(0, 5));
  }
  function move(id: string, delta: number) { setImages((current) => moveImage(current, id, delta)); }
  function remove(id: string) { setImages((current) => { const found = current.find((item) => item.id === id); if (found) URL.revokeObjectURL(found.url); return current.filter((item) => item.id !== id); }); }
  return <div><div className="mb-3 flex items-center justify-between"><Label className="text-white/70">{title}</Label><span className="text-xs text-white/35">{images.length}/5</span></div><label htmlFor="property-images" className="block cursor-pointer border border-dashed border-[#d7b45a]/45 bg-[#d7b45a]/[.04] p-5 text-center transition hover:border-[#d7b45a] hover:bg-[#d7b45a]/[.08]" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); addFiles(event.dataTransfer.files); }}><input id="property-images" type="file" accept="image/png,image/jpeg,image/webp" multiple className="hidden" onChange={(event) => { addFiles(event.target.files); event.currentTarget.value = ""; }}/><UploadCloud className="mx-auto text-[#d7b45a]" size={24}/><div className="mt-2 text-sm text-white/75">{hint}</div><div className="mt-1 text-xs text-white/40">{note}</div></label>{uploadErrors.length > 0 && <div role="alert" className="mt-3 border border-red-300/25 bg-red-400/10 p-3 text-xs leading-5 text-red-100">{uploadErrors.map((error) => <div key={error}>{error}</div>)}</div>}{images.length > 0 && <div className="mt-3 grid grid-cols-3 gap-2">{images.map((image) => <div key={image.id} className="group relative aspect-square overflow-hidden border border-white/10 bg-[#07111f]"><img src={image.url} alt={lang === "ar" ? `معاينة ${image.name}` : `Preview ${image.name}`} className="h-full w-full object-cover"/><div className="absolute bottom-1 left-1 flex gap-1 opacity-0 transition group-hover:opacity-100"><button type="button" aria-label={lang === "ar" ? "تحريك للأعلى" : "Move up"} onClick={() => move(image.id, -1)} className="bg-[#07111f]/85 p-1 text-white"><ChevronUp size={12}/></button><button type="button" aria-label={lang === "ar" ? "تحريك للأسفل" : "Move down"} onClick={() => move(image.id, 1)} className="bg-[#07111f]/85 p-1 text-white"><ChevronDown size={12}/></button></div><button type="button" aria-label={removeLabel} onClick={() => remove(image.id)} className="absolute right-1 top-1 bg-[#07111f]/85 p-1 text-white opacity-0 transition group-hover:opacity-100"><X size={14}/></button></div>)}</div>}</div>;
}
